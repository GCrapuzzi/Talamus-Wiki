...
| Benchmark suite Ph1 + RS5 | **Competitive shootout harness** shipped (`benchmarks/`, dev-only, deps never in product): RetrievalSystem protocol + adapters (talamus-search/-smart, bm25, vectordb=MiniLM+FAISS), metrics (recall/MRR/hit/nDCG), BEIR lite loader, tiered `run.py`, CI fakes. **First REAL head-to-head on BEIR SciFact (300 queries)**: talamus-search recall@10 **0.776** / hit **0.793** = bm25 0.776/0.797 ≈ vectordb 0.783/0.793 — **lexical TIES dense embeddings on recall/hit with ZERO embedding infra** (the thesis, measured). Honest: talamus-search trails BM25 on nDCG/MRR (ranking) on monolingual scientific text; SciFact is the dense model's best case / ours worst (no cross-language, no moats). Report: dev/research/2026-06-rs5-competitive-shootout.md |
