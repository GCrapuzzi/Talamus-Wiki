...
| Shootout book hit@10 (`--smart`, engine-dependent) | **0.971** hit / recall **0.929** / nDCG **0.847** with claude-cli expansion (2026-07-08) — leads multilingual-e5 (0.914 / 0.871 / 0.837) on all four; with a free engine (flash-lite) e5 leads ranking | — |
| Search MRR | 0.726 | — |
...
| F1 fresh benchmark round (2026-07-08) | Whole competitive suite re-run on current code. **Book (expansion engine claude-cli):** talamus-smart recall **0.929** / nDCG **0.847** / MRR **0.865** / hit **0.971** — LEADS multilingual-e5 (0.871/0.837/0.857/0.914) on all four incl. ranking; talamus-search byte-identical to June (0.829/0.732/0.727/0.914 → no regression from the C1 refactor). **The honest story is engine-dependent:** with the free flash-lite engine (June artifact) talamus-smart is 0.886/0.783 and e5 leads ranking — so smart's ceiling tracks the LLM you bring. **SciFact (public BEIR):** talamus-search 0.797/0.664/0.628/0.813 — beats BM25 (0.776/0.652) on all four (RS8 adaptive-trigram confirmed, now with a committed artifact), multilingual-e5 leads (0.807/0.694). **Token efficiency re-confirmed:** −97.7% recall / −99.8% search vs load-all (`2026-07-08-token-efficiency.md`). one_screen.py now cites the fresh artifacts + the two-faced honest row; docs/benchmarks.md regenerated. Caveats stated: single runs, expansion nondeterministic (RS4 ~0.06 swing), latencies GPU-contended → scale artifact stays canonical. ask-eval 2026-06-17 still valid (ask path unchanged, retrieval proven identical) |
| UAT round (2026-07-09) | View-by-view UAT on the 212-note book brain (via the preview browser) found + fixed three defects. **(1) Usage-limit misclassification** (product): claude-cli's 2026 phrasing `out of usage credits · resets ...` was not in `_LIMIT_SIGNATURES` → a hit limit surfaced as a generic failure with a misleading "log in" hint. Added `usage credits`/`out of credits`; the probe now sets `limit_reached` + a reset/switch hint. **(2) Engine switch missing from the UI** (the maintainer's explicit gap "non posso switchare provider"): `update_engine_settings` existed in services but no endpoint exposed it — added POST `/api/engines/select` (S1-guarded) + `api.selectEngine` + a "Use this engine" button on every non-active available engine (active marker now strictly `selected_engine`); switch verified live gemini→codex→restored. **(3) Graph hero blank under RAF throttling** (hidden/background tab, low-power): d3's tick loop is RAF-driven so the canvas never painted — now settles 90 ticks synchronously and draws once on mount (RAF still animates when visible); verified `painted: true`. TDD for (1)+(2). Gate ALL GREEN. |
| Ask time-travel + token cost + aesthetic polish (2026-07-09) | Ask view reaches CLI parity on the TIME moat and the cost surface: an `as of` input → `ask_brain(as_of=…)` answers from the brain AS IT WAS at that instant (reads each hit's version current then, synthesizes only from those; empty-past reported honestly — verified live "nothing relevant as of 1990"), route marked `as-of`, an `as of <t>` badge on the answer; the per-answer token cost (already in `AskResult.context_tokens`) is labeled `cost`. Service gains `as_of` param + field; `/api/ask` passes `{question, as_of}`; CLI/MCP unchanged; TDD (past/empty-past/token tests). **Aesthetic pass** (lifts every view at once): one accent focus ring on all inputs/textareas/selects, themed placeholders, thin themed scrollbars. |
| UX debt closed (2026-07-09) | The three UAT-logged gaps closed. **Ask `--trace` parity:** `AskResult.trace` now carries a readable subset (expanded query, domains routed, notes read into context — `_public_trace` drops internal keys); a collapsible "How this answer was found" panel surfaces it (both the routed and as-of paths populate it). **Home reindex:** new POST `/api/reindex` (S1-guarded; service `reindex_brain` wraps `store.reindex`) + a "Reindex now" action that appears only when the cache is stale and refreshes readiness on success — verified live (212 notes rebuilt, tokenless→403). The stale-cache stat is no longer a dead end. **Review clarity:** the raw candidate id (`ontology_candidate-20260612-…`) removed from the card (kind label stays, id → hover title; the created date already shows the when) — verified no raw id renders. TDD for both backend additions; gate ALL GREEN; SPA rebuilt |
