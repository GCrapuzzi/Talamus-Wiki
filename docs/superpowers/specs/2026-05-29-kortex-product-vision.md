# Kortex — Visione di Prodotto (bussola a 6 mesi)
...
sarà Kortex nella sua **versione finale a 6 mesi**, in modo che ogni scelta tecnica
...
**Kortex è una memoria unica, local-first, condivisa tra una persona e i suoi
agenti AI.**
...
Concettualmente Kortex è un **compilatore di conoscenza**: trasforma materiale
...
2. L'agente, durante il lavoro, consulta Kortex ("cosa sappiamo già su X?").
3. A fine sessione, la conversazione e il lavoro prodotto (codice, modifiche)
   vengono depositati in Kortex.
...
studenti che usano Kortex come second brain classico (alimentazione manuale +
domande citate). Funziona per loro, ma non è il fuoco dei primi 6 mesi.

**Rimandati a dopo i 6 mesi:** sviluppatori terzi che integrano Kortex come backend
...
**Dove Kortex è unico (in ordine di punta):**
...
5. **Semplice e leggero** — gira anche su un PC di media potenza. Zep vuole un
   database a grafo, mem0 vuole Qdrant + Postgres o il cloud; **Kortex gira su un
...
relazione porta **due assi temporali**: *quando era vero nel mondo* e *quando Kortex
...
- **Indici derivati e rigenerabili**: grafo, ricerca, ontologia e cache vivono in
  un'area tecnica gestita (`.kortex/`, con una cache leggera tipo SQLite). Si possono
...
- **Layout:** le schede stanno in cartelle in chiaro e modificabili; materiale
  grezzo, indici, cache e log stanno nell'area gestita `.kortex/`.
- **Git obbligatorio e automatico:** Kortex committa ai checkpoint significativi
...
- **Come si revisiona a 6 mesi** (UI ancora assente): un comando (`kortex review`)
...
- **Interrogazione diretta di Kortex** (come dalla futura UI) → Kortex **confeziona
  la risposta citata** usando il modello LLM scelto.
- **Dentro un agente** → Kortex fornisce solo le **risorse** (schede + fonti) e
  risponde l'LLM dell'agente, senza doppioni.

## 13. Gli agenti come bibliotecari

Due livelli di accesso, non uno solo:
- **Livello facile — "chiedi e basta":** una domanda, Kortex fa il giro e restituisce
  le schede giuste.
...
**Porte di prima classe a 6 mesi:** CLI (comandi `kortex ...`), SDK Python (la base su
...
- **Interrogazione conversazionale**: chiedere a Kortex di trovare o modificare una
...
- Quando si usa un modello **cloud**, i contenuti elaborati vengono inviati a quel
  modello: Kortex lo dichiara con un **avviso chiaro**.
...
- **Nessun lock-in:** le note si aprono e si leggono senza Kortex e senza Obsidian.
