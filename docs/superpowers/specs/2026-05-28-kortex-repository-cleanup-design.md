# Kortex Repository Cleanup Design
...
Canonical product name: `Kortex`.

Code and packaging names:

- Python package: `kortex`
- CLI command: `kortex`
- PyPI/project name: `kortex`
- Config filename: `kortex.json`
- Default project status text: `kortex project status ok`
- Agent skill name: `kortex-knowledge`
...
```text
src/brain/ -> src/kortex/
```

Tests:

```text
tests/test_brain_ask.py -> tests/test_kortex_ask.py
tests/test_brain_cli.py -> tests/test_kortex_cli.py
tests/test_brain_graph.py -> tests/test_kortex_graph.py
tests/test_brain_models.py -> tests/test_kortex_models.py
tests/test_brain_obsidian_renderer.py -> tests/test_kortex_obsidian_renderer.py
tests/test_brain_paths_config.py -> tests/test_kortex_paths_config.py
tests/test_brain_search.py -> tests/test_kortex_search.py
```

Skill:

```text
skills/brain-knowledge/ -> skills/kortex-knowledge/
```

## Reference Updates

Update all active references from `brain` to `kortex` where they describe the
product, package, CLI, config, tests, or skill.

Examples:

- `from brain...` becomes `from kortex...`
- `python -m brain.cli` becomes `python -m kortex.cli`
- `brain init` becomes `kortex init`
- `brain.json` becomes `kortex.json`
- `BrainPaths` can either become `KortexPaths` or remain temporarily if the
  implementation plan chooses minimal API churn. Preferred result is
  `KortexPaths`, `KortexConfig`, and `KortexCliTests`.
- `Brain Knowledge Skill` becomes `Kortex Knowledge Skill`

Historical docs may mention the old name only when explicitly describing the
cleanup history. User-facing docs should use Kortex.

## Documentation Updates

`AGENTS.md` should describe the repository as Kortex, not the old personal
workspace.

`CLAUDE.md` should be rewritten for Kortex development. It should not mention
...
- beginner CLI examples using `kortex`
- graph is an index, not source truth
- Apache-2.0 license

Update `docs/agent-tool-calling.md` to use Kortex commands and terminology.
...
- all commands exit `0`
- `$tmp/kortex.json` exists
- `$tmp/knowledge/notes` exists
- output uses `kortex`, not `brain`
...
Risk: renaming `brain` to `kortex` breaks imports or console entrypoints.
...
- Package imports use `kortex`.
- CLI command is `kortex`.
- Config file is `kortex.json`.
- Tests are named `test_kortex_*`.
- Legacy tracked folders are removed.
- Product docs and agent protocol use Kortex terminology.
- Kortex test suite passes.
- The repository can be physically renamed to `Kortex` after tools are closed.
